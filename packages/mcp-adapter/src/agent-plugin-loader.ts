	...
	for (const pluginPath of getPluginPaths(paths)) {
		...
		for (const [serverName, server] of Object.entries(loaded.mcpServers)) {
			if (mcpServers[serverName]) {
				console.warn(
					`Agent Plugin at ${resolvePluginPath(pluginPath, cwd)} skips duplicate normalized MCP server ${serverName}`,
				);
				continue;
			}
...
function translateAgentPluginMcpConfig(raw: unknown, manifest: AgentPluginManifest, pluginRoot: string): McpConfig {
	...
	for (const [serverName, entry] of Object.entries(mcpConfig.mcpServers)) {
		...
		if (mcpServers[normalizedName]) {
			console.warn(
				`Agent Plugin ${manifest.name} skips invalid MCP server ${serverName}: normalized server name ${normalizedName} already exists`,
			);
			continue;
		}
...
function translateStdioServer(
	...
	const command = raw.command.startsWith("./")
		? resolveContainedPath(pluginRoot, raw.command, pluginRoot)
		: raw.command;
		...
		env: {
			...Object.fromEntries(
				Object.entries(env).map(([key, value]) => [
					key,
					expandPluginPlaceholders(value, pluginRoot, pluginDataDir),
				]),
			),
...
function translateEnv(
	...
	for (const [key, entry] of Object.entries(value)) {
		if (key === "PLUGIN_ROOT" || key === "PLUGIN_DATA") {
			console.warn(
				`Agent Plugin ${manifest.name} skips invalid MCP server ${serverName}: env must not define ${key}`,
			);
			return null;
		}
...
function translateHeaders(
	...
	try {
		...
		console.warn(
			`Agent Plugin ${manifest.name} skips invalid MCP server ${serverName}: headers are not valid HTTP fields`,
		);
		return null;
	}
...
function isBareCommand(command: string): boolean {
	return (
		!command.includes("/") &&
		!command.includes("\\") &&
		!command.includes(PLUGIN_ROOT_PLACEHOLDER) &&
		!command.includes(PLUGIN_DATA_PLACEHOLDER)
	);
}
...
function resolvePluginCwd(value: unknown, pluginRoot: string, pluginDataDir: string): string | null {
	...
	if (value === PLUGIN_ROOT_PLACEHOLDER || value.startsWith(`${PLUGIN_ROOT_PLACEHOLDER}/`)) {
		return resolveContainedPath(pluginRoot, value.replace(PLUGIN_ROOT_PLACEHOLDER, "."), pluginRoot);
	}
	if (value === PLUGIN_DATA_PLACEHOLDER || value.startsWith(`${PLUGIN_DATA_PLACEHOLDER}/`)) {
		return resolveContainedPath(pluginDataDir, value.replace(PLUGIN_DATA_PLACEHOLDER, "."), pluginDataDir);
	}
	return null;
}
...
function expandPluginPlaceholders(value: string, pluginRoot: string, pluginDataDir: string): string {
	return value.replaceAll(PLUGIN_ROOT_PLACEHOLDER, pluginRoot).replaceAll(PLUGIN_DATA_PLACEHOLDER, pluginDataDir);
}
