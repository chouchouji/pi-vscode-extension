...
export async function runMcpScript(
	...
	const resolvedTimeoutMs =
		Number.isFinite(timeoutMs) && timeoutMs > 0 ? Math.floor(timeoutMs) : DEFAULT_MCP_SCRIPT_TIMEOUT_MS;
	...
	const callTool = async (path: string, args?: Record<string, unknown>) => {
		...
		const index =
			calls.push({ operation: "call", path, ok: false, error: "incomplete", durationMs: 0, startedAt }) - 1;
