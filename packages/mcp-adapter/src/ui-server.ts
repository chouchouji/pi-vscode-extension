...
import { buildCspMetaContent, buildHostHtmlTemplate } from "./host-html-template.ts";
...
export async function startUiServer(options: UiServerOptions): Promise<UiServerHandle> {
	...
	const isAppOnlyTool = (toolName: string): boolean => {
		const toolDefinition = options.manager
			.getConnection(options.serverName)
	...
	const server = http.createServer(async (req, res) => {
		try {
			...
			if (url.pathname === "/proxy/tools/call") {
				...
				const definition =
					options.config?.mcpServers[options.serverName] ?? options.state?.config.mcpServers[options.serverName];
				...
				const approval = options.state
					...
					: options.config &&
							isToolCallApprovalRequired(options.config, options.serverName, toolMeta, approvalMetadata)
				...
				try {
					...
					const result = options.config
						? await withSessionRecovery(
								...
								options.serverName,
								(conn) =>
									conn.client.callTool(callArgs, options.manager.getRequestOptions?.(options.serverName)),
							)
			...
			if (url.pathname === "/proxy/ui/context") {
				...
				if (
					(content !== undefined &&
						(!Array.isArray(content) || content.some((block) => !ContentBlockSchema.safeParse(block).success))) ||
...
function resolvePortCandidates(port: number | undefined): number[] {
	...
	for (let offset = 0; offset < count; offset += 1) {
		const candidate =
			MOSHI_DISCOVERY_PORT_START + ((nextMoshiDiscoveryPort - MOSHI_DISCOVERY_PORT_START + offset) % count);
...
function validateTokenQuery(url: URL, expected: string, res: ServerResponse, parameter = "session"): boolean {
