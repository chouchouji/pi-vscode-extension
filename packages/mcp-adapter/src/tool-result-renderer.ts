...
class CompactMcpToolResult implements Component {
	...
	constructor(title: string, inputPreview: string, display: McpToolResultDisplay, theme: RenderTheme) {
		this.title = title;
...
export function resolveMcpToolRenderOptions(settings?: McpToolRenderSettings): McpToolRenderOptions {
	...
	return {
		...
		collapsedResultLines:
			collapsedLines === 1 || collapsedLines === 2 || collapsedLines === 3 ? collapsedLines : defaultLines,
	};
}
