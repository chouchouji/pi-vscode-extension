...
export function isToolCallApprovalRequired(
	...
	if (serverApproval !== undefined) {
		...
		return approval.some(
			(pattern) =>
				matchesToolPattern(legacyCandidates, [pattern]) && !matchesToolPattern(otherCurrentCandidates, [pattern]),
		);
	}
...
export async function ensureToolCallApproved(
	...
	if (!state.approvedToolCalls) {
		state.approvedToolCalls = new Map<string, true>();
	}
	const approvedToolCalls = state.approvedToolCalls;
