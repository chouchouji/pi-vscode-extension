...
export class McpLifecycleManager {
	...
	private readonly hasPendingAuthForServer: (serverName: string) => boolean;
	...
	private async checkKeepAliveConnections(signal?: AbortSignal): Promise<void> {
		if (this.stopped || signal?.aborted) return;
		await parallelLimit([...this.keepAliveServers], KEEP_ALIVE_CHECK_CONCURRENCY, ([name, definition]) =>
			this.checkKeepAliveConnection(name, definition, signal),
		);
	}
